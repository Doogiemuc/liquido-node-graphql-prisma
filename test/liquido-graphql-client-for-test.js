...
// =============== MOCKs =============
const config = {
	LIQUIDO_API_URL: "http://localhost:4000/graphql"
}
...
// mock our fine cache. No caching at all. Always call backend for every get()
class PopulatingCache {
	...
	put(path, obj) {
		console.log("Mock PollsCache.put(path=" + JSON.stringify(path) +", obj=" + JSON.stringify(obj) + ")")
	}

	get(path) {
		let parsedPath = this.parsedPath(path)
		return this.fetchFunc(parsedPath)
	}
...
const JQL_TEAM_MEMBER = `{ joinedAt role member ${JQL_USER} }`
const JQL_PROPOSAL =  `{ id title description icon status createdAt numSupporters createdBy ${JQL_USER} }`
const JQL_POLL = `{ id title status votingStartAt votingEndAt proposals ${JQL_PROPOSAL} winner ${JQL_PROPOSAL} numBallots { data } }`
